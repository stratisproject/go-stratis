...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --auroria instead)",
		Value:    ethconfig.Defaults.NetworkId,
		Category: flags.EthCategory,
	}
	MainnetFlag = &cli.BoolFlag{
		Name:     "mainnet",
		Usage:    "Stratis mainnet",
		Category: flags.EthCategory,
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(AuroriaFlag.Name) {
			return filepath.Join(path, "auroria")
		}
		return path
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	flags.CheckExclusive(ctx, MainnetFlag, DeveloperFlag, AuroriaFlag)
	...
	case ctx.Bool(AuroriaFlag.Name):
		...
		cfg.Genesis = core.DefaultAuroriaGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.AuroriaGenesisHash)
	case ctx.Bool(DeveloperFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeBeaconLightConfig(ctx *cli.Context) bparams.ClientConfig {
	...
	flags.CheckExclusive(ctx, MainnetFlag, AuroriaFlag, BeaconConfigFlag)
	switch {
	case ctx.Bool(MainnetFlag.Name):
		config.ChainConfig = *bparams.MainnetLightConfig
	case ctx.Bool(AuroriaFlag.Name):
		config.ChainConfig = *bparams.AuroriaLightConfig
	default:
		if !customConfig {
			config.ChainConfig = *bparams.MainnetLightConfig
		}
